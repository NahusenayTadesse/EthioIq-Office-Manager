...
<script lang='ts'>
   ...
   let positionsList = $derived([...new Set(list.map(employee => employee.position))]);
   let genders = $derived([...new Set(list.map(employee => employee.gender))]);
   let filter = $state(false);
   let activeList = $derived([...new Set(list.map(employee => employee.isActive))]);
...
<div class="flex flex-col gap-4 pb-4" transition:fly={{x:-200, duration: 600}}>
   <button onclick={()=> list = mainlist} class="{submitButton} !w-[120px] flex flex-row gap-2 justify-center items-center" title="Clear Filter">
  ...
  <tr>
    <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">
      No employees found matching "{searchQuery}".
       ...
       {:else}
      {#each Object.values(filterEmployees(list, searchQuery)) as employee} 
